...
import {
  BG, SURF, BORD, GOLD, MUTED, TX, INF, WARN,
  RING_COLORS, OUTER_COLOR
} from "../theme";
import { hrefFor, navigate } from "../router";

const detailStyles = `
  .pw-back { transition: background 200ms ease, border-color 200ms ease; }
...
export default function DetailView({ name }) {
  const fig = DATA[name];
  ...
  if (!fig) {
    return (
      <div style={{ background: BG, minHeight: "100vh", color: TX, fontFamily: "Georgia, serif", padding: "40px", textAlign: "center" }}>
        <style>{detailStyles}</style>
        <p style={{ color: MUTED, fontSize: "18px", marginBottom: "20px" }}>
